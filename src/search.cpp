         ...
         && (!Limits.depth || Threads.main()->rootDepth / ONE_PLY <= Limits.depth))
  {
      ...
      if (Limits.use_time_management())
      {
          if (!Signals.stop && !Signals.stopOnPonderhit)
          {
              ...
              // Effectively gives more time if eval is betwen -4 and 1, with max increase of 4x thinking time at eval -1.5.
              double thinkLonger = 1;
              if (bestValue > -4 && bestValue < 1 )
                  thinkLonger =  1 + (pow( ((-4 - bestValue)*(-1 + bestValue)), 3)) / ((15625/(double)64)/3);
